...
<?php $component->withAttributes(['title' => 'Tableau de bord']); ?>
    ...
    <?php if($canSeeRevenue || $canSeeExpenses): ?>
...
<?php endif; ?>
    ...
    <?php if($canSeeRevenue || $canSeeExpenses || count($charts['topProducts']) > 0): ?>
...
<?php endif; ?>
    ...
    <?php if(($canSeeRevenue && collect($charts['salesLast7Days'])->sum('total') > 0) || count($charts['topProducts']) > 0 || ($canSeeRevenue && count($charts['salesByCategory']) > 0) || ($canSeeExpenses && count($charts['expensesByCategory']) > 0)): ?>
        ...
        <script>
            document.addEventListener('DOMContentLoaded', () => {
                ...
                const sales7d = <?php echo $canSeeRevenue ? \Illuminate\Support\Js::from($charts['salesLast7Days']) : '[]'; ?>;
                const topProducts = <?php echo json_encode($charts['topProducts'], 15, 512) ?>;
                const salesByCategory = <?php echo $canSeeRevenue ? \Illuminate\Support\Js::from($charts['salesByCategory']) : '[]'; ?>;
                const expensesByCategory = <?php echo $canSeeExpenses ? \Illuminate\Support\Js::from($charts['expensesByCategory']) : '[]'; ?>;
