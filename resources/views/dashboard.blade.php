<x-layouts.app title="Tableau de bord">
    ...
    <div class="grid grid-cols-2 gap-3 lg:grid-cols-4">
        <x-stat-card label="Chiffre d'affaires (jour)" :value="\App\Support\Money::format($kpis['revenue_today'])" icon="💰" />
        <x-stat-card label="Chiffre d'affaires (mois)" :value="\App\Support\Money::format($kpis['revenue_month'])" icon="📈" />
        <x-stat-card label="Dépenses (mois)" :value="\App\Support\Money::format($kpis['expenses_month'])" icon="🧾" />
        <x-stat-card label="Résultat estimé" :value="\App\Support\Money::format($kpis['estimated_result'])" tone="brand" icon="✨" />
    </div>
    ...
    <div class="mt-4 grid gap-4 lg:grid-cols-2">
        <x-card>
            ...
            <x-empty-state class="mt-4" icon="👥" title="Aucun client pour le moment." description="Le module Clients arrive en Phase 3." />
        </x-card>

        <x-card>
            <h3 class="text-sm font-semibold text-slate-700">Dernières dépenses</h3>
            <x-empty-state class="mt-4" icon="🧾" title="Aucune dépense pour le moment." description="Le module Dépenses arrive en Phase 5." />
        </x-card>
    </div>
</x-layouts.app>
